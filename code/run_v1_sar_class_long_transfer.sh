...
run_one() {
  ...
  "${python_bin}" "${script_dir}/probe_v1_cross_domain_transfer.py" \
    --gan-checkpoint "${run_root}/${tag}_seed${seed}/epoch_0086.pt" \
    ...
    --output "${run_root}/${tag}_seed${seed}/transfer.json" \
    --seed "${seed}" --device "${V1_ABLATION_DEVICE:-cuda:0}"
}

for seed in 451 9201; do
  run_one L1_long_control "${seed}"
  run_one L1_long_sar_class_1 "${seed}"
done
