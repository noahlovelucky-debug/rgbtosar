#!/usr/bin/env bash
# Run the identity-conditioned v-prediction DDPM without disturbing other jobs.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DATA_ROOT="${DATA_ROOT:-${SCRIPT_DIR}/../A02}"
OUTPUT="${OUTPUT:-${SCRIPT_DIR}/runs/conditional_ddpm64_identity_v2_short_20260905}"
EPOCHS="${EPOCHS:-20}"
...
exec torchrun --standalone --nproc-per-node=8 train_conditional_sar_diffusion.py \
  ...
  --epochs "${EPOCHS}" --epoch-size 24000 --batch-size 8 --workers 4 \
  --base 64 --rgb-base 32 --token-dim 256 \
  --diffusion-steps 1000 --condition-drop-prob .10 \
  --lr 0.0002 --weight-decay 0.0001 --ema-decay .999 \
  --identity-loss-weight .10 --view-consistency-weight .25 \
  --sample-steps 50 --guidance-scale 1.5 --preview-every 1 --save-every 5 \
  --seed 20260905
