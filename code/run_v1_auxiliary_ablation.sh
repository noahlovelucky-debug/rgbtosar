...
run_one() {
  ...
  "${python_bin}" "${script_dir}/train_continuous_spatial_v1_ablation.py" \
    ...
    --initialise-checkpoint "${run_root}/P1_seed${seed}_control/epoch_0082.pt" \
    --parent-epoch 82 \
    ...
    --output "${run_root}/${tag}_seed${seed}" \
...
for seed in 2718 451 9201; do
  run_one "${seed}" R1_no_cross_view --cross-view-weight 0
done
for seed in 2718 451 9201; do
  run_one "${seed}" F1_no_feature_match --feature-match-weight 0
done
for seed in 2718 451 9201; do
  run_one "${seed}" T1_no_statistics --statistics-weight 0
done
